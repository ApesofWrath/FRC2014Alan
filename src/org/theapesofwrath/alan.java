 ...
 *
 * Driver stick 1 interchangeable with driver stick 2 2 : Shift Down 3 : Shift
 * Up 4: Auto Aim same as 5 5 : Auto Aim same as 4 8 : Arcade 9 : Tank
 *
 * Driver stick 1 interchangeable with driver stick 2 2 : Shift Down 3 : Shift
 * Up 4 : Auto Aim same as 5 5 : Auto Aim same as 4 8 : Arcade 9 : Tank
 *
 * Operator Stick : 1 : Fire 2 : Top Goal 4 : Left Middle Goal 5 : Right Middle
 * Goal 6 : Shooter Up 7 : Shooter Down 10 : Dump same as 11 11 :Dump same as 10
 * z : Blast/manual mode
 *
 * Steering Wheel Any Button : enable the wheel
 */
...
public class alan extends SimpleRobot {
    ...
    final double ROT_CONV_PULSE = (60.0 / 24.0) * (1.0 / 128.0) * 1.0; // Gear Ratio * 1/(ticks per Rev.) * 1 sec/sec
    ...
    int rpm, target;
    PIDController shooterPID = new PIDController(kP, kI, kD, shooterEncoder, shooterMotorVirtual, sampleRate);

    //AxisCamera camera = AxisCamera.getInstance("10.6.68.11"); // FIXUP
     ...
     */
        ...
        getWatchdog().setEnabled(false); // FIXUP
        air.start();
        if (MAX_RPM > 0) {
            shooterPID.setInputRange(0, MAX_RPM);
        } else {
            shooterPID.setInputRange(MAX_RPM, 0);
        }
        shooterPID.setOutputRange(-1.0, 0);
        //shooterPID.setOutputRange(0, -1.0);
        shooterPID.setAbsoluteTolerance(SHOOTER_TOLERANCE);
        shooterPID.setSetpoint(0);
        shooterPID.enable();

        shooterEncoder.setDistancePerPulse(ROT_CONV_PULSE);
    ...
    public void operatorControl() {
        ...
        while (isOperatorControl() && isEnabled()) {
            ...
            pneumatics();
            //shooterStateMachine();
            //shooterMotorVirtual.set((1 - operatorStick.getZ()) / 2);
            shooterPID.setSetpoint(MAX_RPM * (1 - ((1 - operatorStick.getZ()) / 2)));
            if ((f++ % 1000) == 0) {
                System.out.print(MAX_RPM * (1 - ((1 - operatorStick.getZ()) / 2)));
                System.out.print("  ");
                System.out.print(shooterPID.getSetpoint());
                System.out.print("  ");
                System.out.print(shooterMotorVirtual.get());
                System.out.print("  ");
                System.out.print(shooterPID.getP() + " " + shooterPID.getI() + " " + shooterPID.getD());
                System.out.print("  ");
                System.out.print(shooterEncoder.pidGet());
                System.out.print("  ");
                System.out.println(shooterPID.get());
            }
            //Timer.delay(0.01);
        }//end while loop
    }//end operatorControl
    ...
    private void shooterStateMachine() {
        ...
        switch (shootingState) {
            ...
            case idle:
                ...
                shooterPID.setSetpoint(0);
                System.out.println(String.valueOf(operatorStick.getRawButton(operatorStartStateMachineButton)));
                if (operatorStick.getRawButton(operatorStartStateMachineButton)) {
            ...
            case setManualShoot:
                ...
                //System.out.println(String.valueOf(MAX_RPM * (-(1 - operatorStick.getThrottle()) / 2)));
                shooterPID.setSetpoint(MAX_RPM * ((1 + operatorStick.getZ()) / 2));
                break;
